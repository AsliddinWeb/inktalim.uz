  ...
  <Form :validation-schema="schema" :initial-values="initial" @submit="onSubmit">
    <div class="space-y-4">
      ...
      <Field name="title" v-slot="{ field, errors }">
        <AppInput v-bind="field" label="Kurs nomi *" placeholder="Masalan: Python asoslari" :error="errors[0]" />
      </Field>
      ...
      <Field name="description" v-slot="{ field, errors }">
        <div>
          ...
          <textarea
            ...
            class="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-900/50 border rounded-xl text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 resize-none transition-all duration-200"
...
// Thumbnail URL — vee-validate tashqarisida boshqariladi
const thumbnailUrl = ref(props.course?.thumbnail_url ?? null)

function onThumbnailUploaded(url) {
...
const schema = yup.object({
  title:        yup.string().min(3, 'Kamida 3 ta belgi').required('Kurs nomi majburiy'),
